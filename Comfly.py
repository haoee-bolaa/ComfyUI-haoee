    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "api_base": (["haoee", "ip"], {"default": "haoee"}),
...
class Comfly_kling_text2video:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "prompt": ("STRING", {"multiline": True}),
                "model_name": (["kling-v2-6", "kling-v2-5-turbo"], {"default": "kling-v2-5-turbo"}),
    ...
    FUNCTION = "generate_video"
    CATEGORY = "haoee/Comfly_kling"

    def __init__(self):
        super().__init__()
        self.timeout = 300

    def generate_video(self, prompt, model_name, cfg_scale, aspect_ratio, mode, duration, negative_prompt="", seed=0, api_key=""):
        if api_key.strip():
            self.api_key = api_key
...
class Comfly_kling_image2video:
    ...
    def generate_video(self, image, prompt, model_name, mode, duration, negative_prompt="", seed=0, api_key=""):
        if api_key.strip():
            self.api_key = api_key
        ...
        try:
            ...
            print(f"Sending request with parameters: model={model_name}, duration={duration}")
...
class Comfly_video_extend:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "model_name": (["kling-v2-6", "kling-v2-5-turbo"], {"default": "kling-v2-6"}),
    ...
    def extend_video(self, video_id, model_name, cfg_scale, prompt="", negative_prompt="", api_key=""):
        if api_key.strip():
            self.api_key = api_key
...
class Comfly_sora2_openai:
    ...
    def process(self, prompt, model, apikey="", seconds="15", size="1280x720", image=None, seed=0, private=True):
        ...
        try:
            ...
            response = requests.post(
                "https://maas.haoee.com/v1/videos",
                headers=headers,
                data=data,
                files=files,
                timeout=self.timeout
            ...
            while attempts < max_attempts:
                ...
                try:
                    status_response = requests.get(
                        f"https://maas.haoee.com/v1/videos/{task_id}",
                        headers=self.get_headers(),
                        timeout=self.timeout
                    )
    ...
    def process(self, prompt, model, num_images=1, seed=0, image_way="image",
        ...
        try:
            ...
            if "queue.fal.run" in response_url:
                response_url = response_url.replace("https://queue.fal.run", "https://maas.haoee.com")
            ...
            for i, img_data in enumerate(result_data["images"]):
                if "url" in img_data:
                    ...
                    if "queue.fal.run" in img_url:
                        img_url = img_url.replace("https://queue.fal.run", "https://maas.haoee.com")
...
class Comfly_MiniMax_video:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "prompt": ("STRING", {"multiline": True}),
                "model": (["MiniMax-Hailuo-2.3", "MiniMax-Hailuo-2.3-Fast", "MiniMax-Hailuo-02", "T2V-01", "T2V-01-Director", "I2V-01-Director", "I2V-01-live", "I2V-01", "S2V-01"], {"default": "MiniMax-Hailuo-02"}),
        ...
        try:
            ...
            while attempts < max_attempts:
                ...
                try:
                    ...
                    if status_result.get("code") == -1 and isinstance(status_result.get("message"), str) and status_result["message"].strip():
                        match = re.search(r'(\{.*\})', status_result.get("message", ""))
                        if match:
                            status_result = json.loads(match.group(1))
                    if "base_resp" not in status_result or status_result["base_resp"]["status_code"] != 0:
                        error_message = f"Error in status response: {status_result.get('base_resp', {}).get('status_msg', 'Unknown error')}"
                        print(error_message)
...
class OpenAISoraAPIPlus:
    ...
    - 超时时间：600 秒（10 分钟）
    输入参数：
      - base_url: 默认 https://maas.haoee.com/v1
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "base_url": ("STRING", {"default": "https://maas.haoee.com/v1", "multiline": False}),
...
class Comfly_LLm_API:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "api_baseurl": ("STRING", {"multiline": True, "default": "https://maas.haoee.com/v1"}),
                "api_key": ("STRING", {"default": ""}),
    ...
    FUNCTION = "Haoee_run_llmapi"
    CATEGORY = "haoee/LLM"

    def Haoee_run_llmapi(self, api_baseurl, api_key, model, role, prompt, temperature, seed, ref_image=None, video=None):
        if OpenAI is None:
...
class Comfly_wan2_6_API:
    ...
    def convert_image_to_base64(self, image_tensor):
        ...
        try:
            ...
            print(f"[Haoee_WanVideo INFO] Original image size: {original_size[0]}x{original_size[1]}")
            ...
            if max(original_size) > max_dimension:
                ...
                print(f"[Haoee_WanVideo INFO] Resized image to: {new_size[0]}x{new_size[1]}")
            ...
            for format_name, mime_type, save_kwargs in formats_to_try:
                try:
                    ...
                    if len(image_bytes) < smallest_size:
                        ...
                        print(f"[Haoee_WanVideo INFO] {format_name} format: {base64_size_mb:.2f}MB base64")
                    ...
                    print(f"[Haoee_WanVideo WARNING] Failed to save as {format_name}: {format_error}")
                    continue
            
            if best_result:
                final_size_mb = len(best_result.split(',')[1]) / (1024 * 1024)
                print(f"[Haoee_WanVideo INFO] Final base64 size: {final_size_mb:.2f}MB")
                
                if final_size_mb > 3.0:
                    print(f"[Haoee_WanVideo WARNING] Base64 data is large ({final_size_mb:.2f}MB), may cause API issues")
            ...
            print(f"[Haoee_WanVideo ERROR] Image base64 conversion error: {str(e)}")
            return None
    ...
    def generate_video(self, prompt, api_key, resolution, duration, image=None, audio_url="", prompt_extend=True, shot_type="multi", audio_enabled=True):
        ...
        try:
            ...
            if image is not None:
                ...
                print(f"[Haoee_Wan26_I2V INFO] Image converted to base64 successfully")
            ...
            print(f"[Haoee_WanVideo INFO] Request body: {json.dumps(log_body, indent=2, ensure_ascii=False)}")
            print(f"[Haoee_WanVideo INFO] Sending request to: {url}")
            ...
            if response.status_code == 200:
                ...
                if task_id:
                    print(f"[Haoee_WanVideo INFO] Task created. Task ID: {task_id}")
                    
                    # Poll for result
                    video_url = self.poll_task_status(task_id)
                    
                    if video_url:
                        print(f"[Haoee_WanVideo INFO] Downloading video from: {video_url}")
                        video_adapter = ComflyVideoAdapter(video_url)
                    ...
                    print(f"[Haoee_WanVideo ERROR] No task ID: {result}")
                ...
                print(f"[Haoee_WanVideo ERROR] API error ({status_code}): {error_message}")
                return (EmptyVideoAdapter(), f"API error: {status_code} - {error_message}", "")
                
        except Exception as e:
            print(f"[Haoee_WanVideo ERROR] Video generation error: {str(e)}")
    ...
    def poll_task_status(self, task_id):
        ...
        while time.time() - start_time < max_poll_time:
            try:
                print(f"[Haoee_WanVideo INFO] Querying task: {query_url}")
                ...
                if response.status_code == 200:
                    ...
                    if task_status == 'SUCCEEDED':
                        ...
                        if video_url:
                            print(f"[Haoee_WanVideo INFO] Video ready: {video_url}")
                            return video_url
                        else:
                            print(f"[Haoee_WanVideo ERROR] No video URL: {result}")
                            return None
                    elif task_status == 'FAILED':
                        error_msg = result.get('output', {}).get('message', 'Unknown error')
                        print(f"[Haoee_WanVideo ERROR] Task failed: {error_msg}")
                        return None
                    elif task_status in ['PENDING', 'RUNNING']:
                        elapsed = time.time() - start_time
                        remaining = max_poll_time - elapsed
                        print(f"[Haoee_WanVideo INFO] Status: {task_status} ({elapsed:.1f}s elapsed, {remaining:.1f}s remaining)")
                        time.sleep(poll_interval)
                        continue
                    else:
                        print(f"[Haoee_WanVideo WARNING] Unknown status: {task_status}")
                        time.sleep(poll_interval)
                        continue
                else:
                    print(f"[Haoee_WanVideo ERROR] Query failed: {response.status_code}")
                    return None
                    
            except Exception as e:
                print(f"[Haoee_WanVideo ERROR] Query error: {str(e)}")
                return None
        
        print("[Haoee_WanVideo ERROR] Polling timeout")
        return None
...
class Comfly_HaoeeVideo_vidu:
    ...
    def generate_video(self, image, model="viduq2-pro", prompt="", api_key="", is_rec=False, duration=4, seed=0, resolution="720p", 
    ...
    def generate_video(self, prompt, model, resolution="720p", duration=5, ratio="16:9", apikey="", image=None, seed=0):
        ...
        try:
            ...
            if video_url:
                ...
                return (video_adapter, task_id, json.dumps(result))
            ...
            while attempts < max_attempts:
                ...
                try:
                    ...
                    if status in ["succeeded", "success"] and video_url:
                        ...
                        fail_reason = status_result.get("fail_reason", "Unknown error")
                        return (EmptyVideoAdapter(), task_id, json.dumps({"code": "error", "message": f"Video generation failed: {fail_reason}"}))
...
NODE_DISPLAY_NAME_MAPPINGS = {
    # "Comfly_api_set": "好易-API Settings",
    # "OpenAI_Sora_API_Plus": "OpenAI Sora API Plus节点",
    # "OpenAI_Sora_API": "OpenAI Sora API节点",
    # "Comfly_Mj": "Haoee_Mj", 
    # "Comfly_mjstyle": "Haoee_mjstyle",
    # "Comfly_upload": "Haoee_upload",
    # "Comfly_Mju": "Haoee_Mju",
    # "Comfly_Mjv": "Haoee_Mjv",  
    # "Comfly_Mj_swap_face": "Haoee MJ Face Swap",
    # "Comfly_kling_text2video": "Haoee_kling_text2video",
    # "Comfly_kling_image2video": "Haoee_kling_image2video",
    # "Comfly_kling_multi_image2video": "Haoee_kling_multi_image2video",
    # "Comfly_video_extend": "Haoee_video_extend",
    # "Comfly_lip_sync": "Haoee_lip_sync",
    # "ComflyGeminiAPI": "Haoee Gemini API",
    # "ComflySeededit": "Haoee Doubao SeedEdit2.0",
    # "ComflyChatGPTApi": "Haoee ChatGPT Api",
    # "Comfly_sora2_openai": "Haoee_sora2_openai", 
    # "Comfly_sora2": "Haoee_sora2", 
    # "Comfly_sora2_chat": "Haoee_sora2_chat", 
    # "Comfly_sora2_character": "Haoee Sora2 Character",  
    # "ComflyJimengApi": "Haoee Jimeng API", 
    # "Comfly_gpt_image_1_edit": "Haoee_gpt_image_1_edit",
    # "Comfly_gpt_image_1": "Haoee_gpt_image_1", 
    # "ComflyJimengVideoApi": "Haoee Jimeng Video API",
    # "Comfly_Flux_Kontext": "Haoee_Flux_Kontext",
    # "Comfly_Flux_Kontext_Edit": "Haoee_Flux_Kontext_Edit",
    # "Comfly_Flux_Kontext_bfl": "Haoee_Flux_Kontext_bfl",
    # "Comfly_Flux_2_Pro": "Haoee_Flux_2_Pro",
    # "Comfly_Flux_2_Flex": "Haoee_Flux_2_Flex",
    # "Comfly_Googel_Veo3": "Haoee Google Veo3",
    # "Comfly_mj_video": "Haoee MJ Video",
    # "Comfly_mj_video_extend": "Haoee MJ Video Extend",
    # "Comfly_qwen_image": "Haoee_qwen_image",
    # "Comfly_qwen_image_edit": "Haoee_qwen_image_edit",
    # "Comfly_Doubao_Seedream": "Haoee Doubao Seedream3.0",
    # "Comfly_Doubao_Seedream_4": "Haoee Doubao Seedream4.0",
    # "Comfly_Doubao_Seedream_4_5": "Haoee Doubao Seedream4.5",
    # "Comfly_Doubao_Seededit": "Haoee Doubao Seededit3.0",
    # "Comfly_MiniMax_video": "Haoee MiniMax Hailuo Video",
    # "Comfly_suno_description": "Haoee Suno Description",
    # "Comfly_suno_lyrics": "Haoee Suno Lyrics",
    # "Comfly_suno_custom": "Haoee Suno Custom",
    # "Comfly_suno_upload": "Haoee Suno Upload",
    # "Comfly_suno_upload_extend": "Haoee Suno Upload Extend", 
    # "Comfly_suno_cover": "Haoee Suno Cover",
    # "Comfly_vidu_img2video": "Haoee Vidu Image2Video",
    # "Comfly_vidu_text2video": "Haoee_vidu_text2video",
    # "Comfly_vidu_ref2video": "Haoee_vidu_ref2video",
    # "Comfly_vidu_start-end2video": "Haoee_vidu_start-end2video",
    # "Comfly_nano_banana": "Haoee_nano_banana",
    # "Comfly_nano_banana_fal": "Haoee_nano_banana_fal",
    # "Comfly_nano_banana_edit": "Haoee_nano_banana_edit",
    # "Comfly_nano_banana2_edit": "Haoee_nano_banana2_edit",
    # "Comfly_nano_banana2_edit_S2A": "Haoee_nano_banana2_edit_S2A",
    # "ComflyGrok3VideoApi": "Haoee Grok3 Video",
    # "Comfly_Z_image_turbo": "Haoee_Z_Image_Turbo",
